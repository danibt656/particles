'''
Program that runs Conway's Game of Life
'''
...
# Inicializar icono y título de pantalla
# pygame.display.set_icon(pygame.image.load('lifeicon.png'))
...
while True:
    ...
    for event in ev:
        ...
        # Detectar si se ha hecho click, entonces "revivimos" la celda en cuestión
        if sum(mouseClick) > 0:
            posX, posY = pygame.mouse.get_pos()
            celX, celY = int(np.floor(posX / dimCW)), int(np.floor(posY / dimCH))
            newGameState[celX, celY] = not mouseClick[2]

        # Detectamos si hace click en la X para cerrar
    ...
    for y in range(0, nyC):
        for x in range(0, nxC):
            ...
            if not pauseExect:
                ...
                # Rule #1. Una célula muerta con 3 vecinas vivas, "revive"
